...
export function Tree({ children, showLine, ...attrs }: TreeProps) {
  return (
    <div
      {...attrs}
      className={clsx([attrs.className, "tree", showLine && "show-line"])}
    >
      {children}
    </div>
  );
  ...
  return (
    <div className={clsx(["tree-item", props.className])} {...props.attrs}>
      <div
        ...
        onMouseUp={() => {
          props.canClickLabelExtend && handleExpand(true);
        }}
      >
        {hasChild && (
          <button
        ...
        {!hasChild && !props.hideSpece && (
          <span className="tree-item-space"></span>
        )}
        {props.icon && <i className="tree-item-icon">{props.icon}</i>}
        {props.label}
      </div>
      {hasChild && expand && (
        <div className="tree-item-content">{props.children}</div>
      )}
    </div>
  );
}
